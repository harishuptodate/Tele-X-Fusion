...
const isLowContext = (text) => {
	const meaningfulText = removeLinks(text).trim(); // Remove links
...
bot.on('channel_post', async (ctx) => {
	...
	console.log('Logging message post simulation:');
	console.log('Original Message:', textContent);
	console.log('Processed Message:', finalCaption);
	console.log('Chunks:', captionChunks);

	// Simulate API call logging without posting
	for (let i = 0; i < captionChunks.length; i++) {
		console.log(`Simulated API call for chunk ${i + 1}:`, captionChunks[i]);
	}
